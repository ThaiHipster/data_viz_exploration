...
library(tidyverse)
load("36361-0001-Data.rda") #load() is only for .rda; distinct from a read.x() 
...
ggplot(regular_users)+
  geom_bar(aes(x=HEALTH,fill=HEALTH,y=..count../sum(..count..))) # computed variable: count
...
#coxcomb
ggplot(regular_users)+
  geom_bar(aes(x=HEALTH,fill=HEALTH,y=..count../sum(..count..)),width=1)+
  coord_polar()+
  scale_y_sqrt()
...
ggplot(regular_users)+
  ...
  scale_fill_brewer(palette="RdYlBu",direction=-1,
                    labels=c("Excellent","Very Good","Good","Fair","Poor"),
                    name="Self-Reported \nOverall Health")+
  labs(x="",y="Proportion of Respondants",
...
ggplot(never_users)+
  ...
  scale_fill_brewer(palette="RdYlBu",direction=-1,
                    ...
                    name="Self-Reported \nOverall Health")+
  labs(x="",y="Proportion of Respondants",
       title="Self-Reported Health of Never Users of Marijuana")+
  scale_x_discrete(labels=c("Excellent","Very Good","Good","Fair","Poor"))+
  scale_y_continuous(labels=label_percent())+
  theme_classic()
```
...
ggplot(combined_data)+
  ...
  scale_fill_brewer(palette="RdYlBu",direction=-1,
                    ...
                    name="Self-Reported \nOverall Health")+
  labs(x="",y="Proportion of Respondants",
       title="Self-Reported Health of Never Users of Marijuana vs. Regular Users")+
  scale_x_discrete(labels=c("Excellent","Very Good","Good","Fair","Poor"))+
  scale_y_continuous(labels=label_percent())+
  theme_classic()+
...
Notice that in this graph the total height of all bars is 1 -- we are seeing here the joint distribution of the variables.  If we wanted to see a conditional distribution, we can manually compute the needed values. Let's quickly create a new dataset which contains the numbers we need:
...
Now, the total height of all the "never" bars is 1 and the total height of all the "regular" bars is 1.  We are now seeing the conditional distribution of "HEALTH," conditioned on "user_status."

## Results

From our graphs, we can see that survey respondents who reported never having used marijuana were more likely to describe their overall health as "excellent" than were survey respondents who had used marijuana on at least `r regular_use_criteria` days in the past 30 days.  The percentage of these respondents who described their health as "good," "fair," or "poor" was greater than the corresponding percentages for those who had never used marijuana.  For both groups of respondents, the most commonly given description of their overall health was "very good." 
